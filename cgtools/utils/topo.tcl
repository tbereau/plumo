...
proc ::cgtools::utils::topoforvirtualsites { topo } {
    # Returns topology compatible with virtual sites.
    # Use it right before integrate call.
    variable ::cgtools::virtual_sites
    set molids ""
    foreach top $topo {
        if { [lsearch $molids [lindex $top 0]] == -1 } {
            lappend molids [lindex $top 0]
        }
    }
    set molele ""
    foreach id $molids {
        set virtualID 0
        foreach site $virtual_sites {
            if { [lindex $site 0] == $id } {
                set virtualID 1
            }
        }
        if { $virtualID == 1 } {
            ...
            foreach top $topo {
                if { [lindex $top 0] == $id } {
                    lappend elelist [lrange $top 1 end]
                }
            }
            foreach site $virtual_sites {
                if { [lindex $site 0] == $id } {
                    lappend elelist [lindex $site 1]
                }
            }
            lappend molele [flatten $elelist]
        }
    }
    return $molele
}