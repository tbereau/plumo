...
proc ::cgtools::analysis::ti::setup_ti {  lambdas dlambda {eqtime 0} {delta ""} } {
  ...
  if {[expr $lmbd <= 0.0] || [expr $lmbd >= 1.0]} {
    ::mmsg::err [namespace current] "dlambda must be between 0 and 1"
  }
  foreach li $lmbds {
    if {[expr $li <= 0.0]} {
      ::mmsg::err [namespace current] "Each lambda must be strictly positive"
    }
    # Check that lambda +/- dlambda is within [0,1]
    if { [expr $li - 0.5*$dlmbd < 0.0] || [expr $li + 0.5*$dlmbd > 1.0]} {
...
proc ::cgtools::analysis::ti::analyze_ti { } {
  ...
  if { $currenttime >= $tiequitime } {
    ...
    foreach li $lmbds {
      # Evaluate force field with li+dlmbd
      ::cgtools::forcefield::update_peptide_ff [expr $li + 0.5*$dlmbd]
      set epotp 0.0
      ...
      puts -nonewline $pipe [format "%9.4f " [expr ($epotp-$epotm)/$dlmbd]]
    }
    puts $pipe ""
    close $pipe
